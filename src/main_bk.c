...
int main(int argc, char * argv[])
{
  ...
  L3Cache[numc] = (LLCache*)malloc(sizeof(LLCache));
  }
 ...
 assert((log_base2(NUM_CHANNELS) + log_base2(NUM_RANKS) + log_base2(NUM_BANKS) + log_base2(NUM_ROWS) + log_base2(NUM_COLUMNS) + log_base2(CACHE_LINE_SIZE)) == ADDRESS_BITS);
  // The step below only has to be performed if using physical addr. this works for HMC SPEC2006 Traces
  if (NUMCORES == 1) {
  pow_of_2_cores = 1;
  }
  else {
  ...
  print_params();
  CACHE_SIZE = 1024*2048;//2MB LLC (PRIVATE)
  ...
  for(int i=0; i<NUMCORES; i++)
  {
	  ...
	  ROB[i].optype = (int*)malloc(sizeof(int)*ROBSIZE);
          construct_cache(L3Cache[i], CACHE_SIZE, 8,1, 64);
  }
  os_pages = 2097152;
  ...
  for(numc=0; numc<NUMCORES; numc++)
  {
	     fscanf(tif[numc],"%d %c",&nonmemops[numc],&opertype[numc]);
	     printf("%d, %c ", nonmemops[numc],opertype[numc]);
	      if (fgets(newstr,64,tif[numc])) {
...
/* For each core, bring in new instructions from the trace file to
	  ...
	  if (nonmemops[numc]) {  /* Have some non-memory-ops to consume. */
	    ...
	    num_fetch++;
		  ...
		  long long int wb_addr = 0;
                  ...
                  LINE_STATE* currLine = CacheIsWriteback(L3Cache[numc], numc, instrpc[numc], addr[numc], ACCESS_LOAD);
                  if(currLine != NULL){
                     wb_addr = currLine->phy_addr;
                     wb_inst_addr = currLine->PC;
                  }
                  int L3Hit = LookupAndFillCache(L3Cache[numc], numc, instrpc[numc], addr[numc], ACCESS_LOAD);
		  ...
		  if(lat) {
	        ...
	        if (opertype[numc] == 'W') {
		      ...
		      /* Also, add this to the write queue. */
                      ...
                      int L3Hit = LookupAndFillCache(L3Cache[numc], numc, instrpc[numc], addr[numc], ACCESS_STORE);
                      if(L3Hit){
                      }else{
				if(currLine != NULL){
                             		insert_write(wb_addr, CYCLE_VAL, numc, ROB[numc].tail); 
  ...
  scheduler_stats();
  for (numc=0; numc < NUMCORES; numc++) {
  print_cache_stats(L3Cache[numc]);
  }
  print_stats();  
...
//If static randomization is required
